...
const ACTIVE_TASK = ['delivered', 'in_progress']
...
export function analyze(state) {
  ...
  else if (state.matter.supplierId === 'B') {
    const followups = state.tasks.filter(task => ['T-PUR', 'T-SALES'].includes(task.id))
...
export function getDocuments(state) {
  ...
  const rows = [
    ...
    ['DOC-ROLES', '组织与任务职责样例', '采购经办（procurement）：发起质量核验、采购跟进并提交回执；质量负责人（quality）：核验 B 并提供凭据；销售经办（sales）：同步交期并提交回执；业务负责人（lead）：批准切换和复核关闭。全部岗位为演示角色，收件箱为页面内模拟。'],
    ['DOC-RULES', '供应商切换与关闭规则样例', 'B 质量核验通过且 T-QA 完成后，业务负责人才能人工确认切换，产生 DEC-03，替代 DEC-01 并创建 T-PUR、T-SALES。助手只能预览，不能自动批准。两项任务须分别提交非空证据；业务负责人复核后关闭办公协同事项，不代表到货、生产完成或订单交付。质量拒绝后可重新发起 T-QA 核验，同一任务保留历史。发送失败可重试，每轮核验或任务最多三次送达尝试。'],
    ['DOC-STATE', '事项当前状态与回执', `当前数据版本 ${state.revision}；事项 ${state.matter.id} 状态 ${state.matter.status}，选定供应商 ${state.matter.supplierId}。决定：${state.decisions.map(decision => `${decision.id}=${decision.status}（${decision.text}）`).join('；')}。任务：${state.tasks.length ? state.tasks.map(task => `${task.id} 收件人 ${LABELS[task.assignee]}，状态 ${task.status}，送达尝试 ${task.attempts}，核验结论 ${task.qualityResult ?? '无'}，回执 ${typeof task.receipt === 'string' ? task.receipt : task.receipt?.evidence ?? '无'}`).join('；') : '尚未创建'}。下一步：${analysis.nextStep}`],
...
function check(state, action, role) {
  ...
  switch (action.type) {
    ...
    case 'submit_quality':
      ...
      if (!qa || !ACTIVE_TASK.includes(qa.status)) fail('质量任务尚未送达或已完成。', 'TASK_STATE_CONFLICT', 409)
    ...
    case 'approve_switch':
      roleIs('lead')
      if (state.matter.supplierId === 'B' || state.decisions.some(decision => decision.id === 'DEC-03')) fail('供应商切换已批准。', 'ALREADY_APPROVED', 409)
      if (supplierB.quality !== 'approved' || qa?.status !== 'completed' || qa.qualityResult !== 'approved') fail('供应商 B 尚未通过质量核验，或质量任务未完成。', 'QUALITY_NOT_APPROVED', 409)
    ...
    case 'submit_receipt': {
      ...
      if (!ACTIVE_TASK.includes(task.status)) fail('仅已送达或处理中的任务可以提交回执。', 'TASK_STATE_CONFLICT', 409)
      hasEvidence()
      return { title: '提交任务回执', details: [`${task.id}：${task.title}。`, `回执：${action.evidence.trim()}`, '提交后进入待复核，由业务负责人确认事项关闭。'] }
    }
    case 'close_matter':
      roleIs('lead')
      if (state.matter.supplierId !== 'B' || supplierB.quality !== 'approved' || qa?.status !== 'completed' || !state.decisions.some(decision => decision.id === 'DEC-03' && decision.status === 'effective')) fail('切换尚未完成审批，不能关闭事项。', 'CLOSE_CONDITIONS_UNMET', 409)
      ...
      return { title: '复核并关闭办公事项', details: ['复核采购安排和销售同步两份回执。', '确认后任务标记已完成，SUP-001 办公协同事项关闭。', '关闭仅表示办公协同完成，不表示采购到货、生产完成或订单交付。'] }
    case 'retry_delivery': {
...
export function applyAction(state, action, role) {
  ...
  switch (action.type) {
    case 'request_quality': {
      ...
      if (task) {
        task.history = [...(task.history ?? []), { receipt: task.receipt, qualityResult: task.qualityResult, attempts: task.attempts, completedAt: task.completedAt }]
