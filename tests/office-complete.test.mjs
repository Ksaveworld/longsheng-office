...
test('six independent seeded matters use actual transitions, isolated orders and graph source endpoints',()=>{
 const matters=createSeedMatters({includeArchived:true}).filter(s=>!s.scenario);assert.equal(matters.length,6);assert.equal(new Set(matters.flatMap(s=>s.orders.map(o=>o.id))).size,12);
...
test('unfinished legacy supplier qualification never becomes current plan approval; old closed history remains',()=>{
 const old=createSeedMatters({includeArchived:true})[4];delete old.workflowVersion;delete old.planVersions;delete old.planReviews;delete old.matter.executionApproval;
