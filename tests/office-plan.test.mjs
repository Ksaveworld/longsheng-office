...
const receive=(state,taskId,role)=>act(state,'accept_task',role,{taskId})
const receipt = (state, taskId, role) => act(act(receive(state,taskId,role), 'start_task', role, { taskId }), 'submit_receipt', role, { taskId, evidence: `${taskId} 已沟通的跟进记录；未确认实际到货。` })
const qaPass=state=>qaDone(act(state,'request_quality'),'approved')
const qaDone = (state, result) => act(act(receive(state,'T-QA','quality'), 'start_task', 'quality', { taskId: 'T-QA' }), 'submit_quality', 'quality', { taskId: 'T-QA', result, evidence: `质量检验记录：${result}` })
...
test('A 全链：选择不批准，负责人确认、独立回执和最终复核均可追溯，关闭仍保留风险', () => {
  ...
  assert.equal(previewAction(state, { type: 'approve_keep_a' }, 'lead').allowed, false)
  state=qaPass(state)
...
test('B 必须显式选择，仍保留质量、批准及关闭条件', () => {
  assert.throws(()=>act(createState(),'request_quality'),{code:'PLAN_NOT_SELECTED'})
  for (const initial of [select(createState(), 'B')]) {
...
test('未选择、未批准及越权不产生任务；选择理由可空', () => {
  ...
  assert.throws(() => act(qaPass(selected), 'approve_keep_a', 'lead', { evidence: '' }), { code: 'EVIDENCE_REQUIRED' })
  assert.equal(act(selected,'request_quality').tasks[0].planVersionId,selected.matter.planSelection.planVersionId)
  assert.throws(() => act(selected, 'approve_switch'), { code: 'PLAN_MISMATCH' })
  assert.equal(selected.tasks.length, 0)
})

test('批准前改选使旧核验失效，旧凭据保留，供应商资格不被改写',()=>{
 let state=qaPass(select(createState(),'B'))
 const oldReview=structuredClone(state.planReviews[0])
 state=select(state,'A')
 assert.equal(analyze(state).executionApproved,false)
 assert.throws(()=>act(state,'approve_keep_a'),{code:'QUALITY_NOT_APPROVED'})
 state=qaPass(state)
 assert.deepEqual(state.planReviews[0],oldReview)
 assert.equal(state.planReviews.length,2)
 assert.equal(state.suppliers[1].quality,'pending')
 state=act(act(state,'approve_keep_a'),'send_tasks')
 ...
 let processing=act(select(createState(),'B'),'request_quality')
 processing=receive(processing,'T-QA','quality');processing=act(processing,'start_task','quality',{taskId:'T-QA'})
 processing=select(processing,'A')
 assert.throws(()=>act(processing,'submit_quality','quality',{taskId:'T-QA',result:'approved',evidence:'旧版本迟到核验'}),{code:'PLAN_CHANGED'})
})

test('A 批准后锁定方案并阻止重复任务；发送失败必须重试，回执岗位隔离', () => {
  let state=act(act(qaPass(select(createState(),'A')),'approve_keep_a'),'arm_delivery_failure')
  assert.ok(state.tasks.filter(task => task.id !== 'T-QA').every(task => task.status === 'pending_delivery'))
  state = act(state, 'send_tasks')
  assert.equal(state.tasks.find(t=>t.id==='T-PUR').status, 'delivery_failed')
  assert.throws(() => select(state, 'B'), { code: 'PLAN_LOCKED' })
...
test('A 规则答复要求所选版本核验，并保留到料风险', async () => {
  const selected = select(createState(), 'A')
  const pending = await runOfficeChat({ state: selected, question: '下一步是什么', role: 'lead', mode: 'rules' })
  assert.match(pending.answer, /当前方案版本的质量核验/)
  assert.doesNotMatch(pending.answer, /B 质量核验待完成/)
  const approved = await runOfficeChat({ state: act(qaPass(selected), 'approve_keep_a'), question: '下一步是什么', role: 'lead', mode: 'rules' })
  assert.match(approved.answer, /确认后发送部门任务/)
